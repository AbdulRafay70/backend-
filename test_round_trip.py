"""
Round Trip Flight Search API Test Script
Tests the AIQS flight search API with round trip search
"""
import requests
import json
import asyncio
                        ...
                        if 'response' in data:
                            ...
                            if 'content' in response_data:
                                ...
                                if command == 'FlightSearchRS':
                                    ...
                                    # Check for round trip flights
                                    round_trip_count = 0
                                    for flight in flight_index:
                                        ond_pairs = flight.get('ondPairs', [])
                                        if len(ond_pairs) == 2:
                                            round_trip_count += 1
                                    print(f"  Round trip flights: {round_trip_count}")
...
def create_sample_round_trip_search_request():
    """Create a sample round-trip flight search request"""
    # Search for round trip: Karachi to Dubai on 10 Feb 2026, return 17 Feb 2026
    departure_date = "10-02-2026"
    return_date = "17-02-2026"
            ...
            "criteria": {
                ...
                "ondPairs": [
                    ...
                    },
                    {
                        ...
                        "originLocation": "DXB",  # Dubai
                        "destinationLocation": "KHI"   # Karachi
                    }
                ],
                "preferredAirline": [],
                "nonStop": False,
                ...
                "tripType": "R",  # O=Oneway, R=Round, M=Multi City
                "target": "Test",
                "paxQuantity": {
                    "adt": 1,  # Adult
                    "chd": 0,  # Child
...
def main():
    """Main test function"""
    print("=" * 60)
    print("AIQS Round Trip Flight Search API Test")
    print("=" * 60)
    ...
    # Step 2: Search for round trip flights
    print("\n[STEP 2] Round Trip Flight Search (WebSocket)")
    print("-" * 60)
    search_params = create_sample_round_trip_search_request()
    ...
    if results:
        ...
        output_file = "round_trip_flight_search_results.json"
        ...
        # Display summary
        print("\n" + "=" * 60)
        print("ROUND TRIP SEARCH SUMMARY")
        print("=" * 60)
        
        total_flights = 0
        round_trip_flights = 0
                ...
                if 'content' in response_data:
                    ...
                    if 'searchResponse' in content:
                        ...
                        if 'flightIndex' in search_resp:
                            ...
                            # Count round trip flights
                            rt_count = 0
                            for flight in search_resp['flightIndex']:
                                ond_pairs = flight.get('ondPairs', [])
                                if len(ond_pairs) == 2:
                                    rt_count += 1
                            round_trip_flights += rt_count
                            print(f"  Round trip flights: {rt_count}")
                            ...
                            if search_resp['flightIndex']:
                                ...
                                if 'ondPairs' in first_flight and first_flight['ondPairs']:
                                    ...
                                    print(f"  Sample route: {ond.get('originLocation', 'N/A')} → {ond.get('destinationLocation', 'N/A')}")
        
        print(f"\n📊 Total flights received: {total_flights}")
        print(f"📊 Round trip flights: {round_trip_flights}")
        
        if round_trip_flights > 0:
            print("✅ Round trip flights found successfully!")
        else:
            print("⚠ No round trip flights found in the results.")
