...
const getOrgId = (org) => {
  return org && typeof org === "object" ? org.id : org;
};
...
const TicketBooking = () => {
  ...
  // Ensure shared api has authorization header when token is available
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Require authentication token, but allow calls when organization is missing.
      if (!token) {
  ...
  const cacheKey = `ticketData-${orgId || 'global'}`;
        ...
        // Build organization query suffix only when orgId is present
        const orgSuffix = orgId ? `?organization=${orgId}` : "";
  ...
  useEffect(() => {
    ...
    // Remove placeholder/demo tickets that have no origin/destination
    // Allow tickets with PNR 'N/A' to be shown when they have route/date fields.
    result = result.filter((ticket) => {
      const hasRoute = ticket.origin || ticket.destination;
      return !!hasRoute;
    });

    // Allow tickets without trip_details to be displayed (placeholder card will be shown)
  ...
  useEffect(() => {
    ...
    // Build unique routes from tickets and update routeFilters once.
    // Use functional state update to avoid depending on routeFilters and causing re-renders.
    setRouteFilters((prev) => {
      ...
      Object.keys(uniqueRoutes).forEach((route) => {
        newRouteFilters[route] = prev[route] || false;
      });
