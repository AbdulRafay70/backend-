...
class BankAccountViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        # Debug: log incoming payload and validated_data to diagnose is_company_account handling
        try:
            import logging
        ...
        try:
            # Log what was actually serialized back
            import logging
            logger = logging.getLogger(__name__)
            logger.debug("BankAccount created output: %s", serializer.data)
        except Exception:
            pass
