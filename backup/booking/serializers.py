...
from scripts.serializers import OrganizationSerializer,AgencySerializer, BranchSerializer
...
class BankAccountSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Also accept camelCase or alternate keys from frontend payloads
        # (e.g. `isCompanyAccount` or `isCompany`) so explicit intent is honored.
        ...
        if not created_by and req is not None:
            try:
                # check raw request data for nested created_by object
                raw_created = None
                if isinstance(req.data, dict):
                    raw_created = req.data.get('created_by') or req.data.get('created_by_id')
                if raw_created:
                    # support either numeric id or dict with id
                    cid = None
